...
- Name: 신상현 / Sanghyeon Shin. Positioning line (2026-09-21, v6): 「사용자의 실제 행동에서 답을 찾아, 직접 만들어 배포하는 서비스 기획자」 / "A service planner who finds the answer in what users actually do, then builds and ships." The v6 proof line ("~가 아니라 ~였다" triplet) was retired by v9 (2026-09-22) because it violates the v9 voice rules below; Hero (2026-09-24, option B chosen by him): screen 1 is a cover — the white "Portfolio" wordmark with a meta row only, no claims. Screen 2 is the one and only introduction: color photo (never grayscale; a grayscale oval on black read as a memorial portrait), the positioning sentence as h1, one proof paragraph (12th-cohort frontend track → 13th-cohort staff → as an intern, ran the 2,000-person hackathon from planning through the finals and built and ran the tools on site), and four numbers: 2,500+ students on the community platform he ran (https://likelion.community/, planned and run by him, developed by AXP — never "built it") · 80.8% (0 of 229 unanswered) · 2.2 s · 5 violating teams from a 562-repo sweep. There is no separate "안녕하세요" section; introducing himself twice read as the page starting over. Right under the introduction (top of the #about section) a "Profile." block lists education, training (SSAFY), certifications (SQLD, ADsP, OPIc IH), and military service like a résumé header — he decided basic facts should always be visible (2026-09-24). Certifications are not listed under Awards. Logo: an SH monogram (DM Serif Display, OFL) in which the S stands in for the H's left stem and the crossbar grows from the S's spine; header mark, favicon (white on the accent blue), and the OG card (white, matching the cover). Markers that must survive any edit: 실제 행동 / 직접 만들어 배포 / 기획부터 본선 운영까지. Planning documents remain a strength; never write "기획서 대신".
...
- Main list cards (2026-09-24): each shows a one-line "what it is" summary (the headline alone did not say what the project was) and key impact as before → after where a before state exists. The hackathon card uses the ANIMAL LEAGUE key visual; its project page opens with the Vimeo loop from the Brand Design Lab (credited under the video; poster image only under reduced motion). Awards: one "representative award per project" (FlowPay, SonGeul, dotori), then the rest.
