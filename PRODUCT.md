...
- Main list cards (2026-09-24): each shows a one-line "what it is" summary (the headline alone did not say what the project was) and key impact as before → after where a before state exists. The hackathon card uses the ANIMAL LEAGUE key visual, which is also the first slide of its project carousel. The Vimeo loop was tried and removed (2026-09-24): the source video ends on a white frame. Big impact numbers in one row share one size, fitted to the longest value (src/lib/fit-stat.ts). Awards: one "representative award per project" (FlowPay, SonGeul, dotori), then the rest.
