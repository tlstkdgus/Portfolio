...
- **경력 상세 페이지** — `/career`에서 프로젝트별 배경, 역할, 성과, 교훈을 상세 서술
- **다국어 기본 내장** — `next-intl` 기반 URL 라우팅(`/ko`, `/en`) 완전 분리, 언어 추가 용이
- **설정 파일 하나로 끝** — `site.ts` + 데이터 파일 7개만 수정. 컴포넌트 코드 수정 불필요
- **완성도 높은 디자인** — Framer Motion 애니메이션, 그라데이션 오브, 스크롤 인디케이터 포함
